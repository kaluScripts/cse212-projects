...
public static class SetsAndMaps
{
    ...
    {
        // Use a HashSet to track words we've seen — O(n) overall
        ...
        foreach (var word in words)
        {
            ...
            // Build the reverse of the current word
            var reversed = $"{word[1]}{word[0]}";

            if (seen.Contains(reversed))
            {
                // Found a symmetric pair; emit as "current & reversed"
    ...
    public static Dictionary<string, int> SummarizeDegrees(string filename)
    {
        ...
        foreach (var line in File.ReadLines(filename))
        {
            ...
            // Column 4 is index 3 (zero-based)
            if (fields.Length > 3)
            {
    ...
    {
        // Build a letter-frequency dictionary for word1
        ...
        foreach (var ch in word2)
        {
            ...
            var lower = char.ToLower(ch);
            if (!letterCounts.ContainsKey(lower))
                return false; // Letter not in word1 at all
            letterCounts[lower]--;
    ...
    public static string[] EarthquakeDailySummary()
    {
        ...
        // Build formatted strings: "place - Mag magnitude"
